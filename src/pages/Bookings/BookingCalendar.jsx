...
export default function BookingCalendar() {
  ...
  const { data, isError, refetch } = useBookings()
                          ...
                          <div className="mt-1 flex-1 flex flex-col justify-center">
                            ...
                            <div className="flex gap-1 justify-center mt-1 flex-wrap">
                              {dayBookings.slice(0, 3).map((b, i) => (
                                <div key={i} className={`h-1.5 w-1.5 rounded-full ${statusConfig[b.status]?.color.split(' ')[0] ?? 'bg-gray-300'}`}></div>
                              ))}
                            </div>
                          </div>
                        )}
          ...
          <CardContent className="space-y-3">
            {selectedBookings.length > 0 ? (
              selectedBookings.map(booking => (
                <div key={booking._uuid} className="p-3 border border-slate-200 rounded-lg hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <span className="text-sm font-bold">{booking.id}</span>
                    <Badge className={statusConfig[booking.status]?.color ?? 'bg-gray-100 text-gray-800'}>
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <User className="h-3 w-3" />{booking.customer}
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <MapPin className="h-3 w-3" />{booking.pickup} → {booking.dropoff}
                    </div>
                    <div className="flex items-center gap-2 text-orange-600 font-bold">
                      <DollarSign className="h-3 w-3" />฿{(booking.fare || 0).toLocaleString()}
                    </div>
                  </div>
        ...
        <DialogContent className="max-w-md">
          ...
          {selectedBookingDetail && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><p className="text-muted-foreground">Fare</p><p className="font-medium text-orange-600">฿{(selectedBookingDetail.fare || 0).toLocaleString()}</p></div>
              <div><p className="text-muted-foreground">Status</p><Badge className={statusConfig[selectedBookingDetail.status]?.color ?? 'bg-gray-100 text-gray-800'}>{statusConfig[selectedBookingDetail.status]?.label ?? selectedBookingDetail.status}</Badge></div>
            </div>
          )}
